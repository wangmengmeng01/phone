  ...
  <div class="riskTest p4" :style="{'padding-bottom':act && act<list.length?'1.1rem':''}">
    <div class="item" v-for="(i, index) in list" v-show="index===act">
      <div class="q">
        ...
        <p class="color_main">{{index+1
          <10? '0'+(index+1):index+1}}<span class="f28 color_font-s">/{{list.length}}</span>
        </p>
      </div>
      <ul class="a">
...
//							delete page_params.backurl;
						...
						this.SET_SUCC_PAGE(page_params);
						this.$go('/static/succ',{client:"4",userToken: this.userToken,forNative:"1"});
					})
